...
function buildRecipe(FAA) {
  ...
  return {
    ...
    steps: [
      ...
      { field: FIELD.ENCOUNTER, role: ROLE.INPUT, candidates: css('#patient_mrn') },
...
module.exports = async function run() {
  // ---- Dry run: fills everything but must NOT submit ----
  {
    ...
    const result = await ENGINE.runRow(recipe, row, { dryRun: true, fieldDelayMs: 0, typeCharDelayMs: 0 });
  ...
  // ---- Live run: should submit exactly once ----
  {
    ...
    const result = await ENGINE.runRow(recipe, row, { dryRun: false, fieldDelayMs: 0, typeCharDelayMs: 0 });
  ...
  // ---- No-match supervisor should fail the row cleanly ----
  {
    ...
    const result = await ENGINE.runRow(recipe, badRow, { dryRun: true, fieldDelayMs: 0, autocompleteTimeoutMs: 300, typeCharDelayMs: 0 });
